...
const services = [
  ...
  { name: '2D Plan', slug: '2d-plan', image: '/generated/proc_planning.png', desc: 'Detailed 2D floor plans ensuring solid foundations and accurate mapping.' },
    ...
    <div className="bg-white">
      ...
      <section className="relative h-screen w-full overflow-hidden bg-black">
        <Carousel
          ...
          onSelect={(api) => setActiveIndex(api ? api.selectedScrollSnap() : 0)}
        >
              ...
              <CarouselItem key={index} className="relative w-full h-screen pl-0 overflow-hidden">
                ...
                <div className="relative z-10 h-full flex items-center px-6 md:px-24">
                  <div className="max-w-4xl text-left animate-fade-up">
                    <h1 className="font-display text-4xl md:text-8xl font-bold text-white mb-4 md:mb-8 leading-tight drop-shadow-2xl text-shadow-lg uppercase tracking-tighter mt-12 md:mt-0">
                      {slide.heading}
                    </h1>
                    
                    <div className="mb-6 md:mb-10 inline-block">
                       <div className="bg-accent text-primary font-bold px-4 md:px-6 py-2 rounded-lg text-base md:text-2xl shadow-xl">
                          {slide.price}
                       </div>
                       <p className="text-white/60 text-[10px] md:text-xs mt-2">*T&C Apply</p>
                    </div>
        ...
        <div className="absolute right-0 top-1/2 -translate-y-1/2 z-20 flex flex-col gap-1 scale-75 md:scale-100 origin-right">
      ...
      <section className="py-12 md:py-24 bg-white relative">
        <div className="container mx-auto px-4 grid md:grid-cols-2 gap-8 md:gap-16 items-center">
          ...
          <div className="animate-fade-up">
            ...
            <h2 className="text-3xl md:text-7xl font-bold mb-4 md:mb-8 text-primary leading-tight uppercase">Elite Living Redefined</h2>
            <p className="text-base md:text-lg text-gray-600 mb-6 md:mb-8 leading-relaxed">
              Galaxy Interior is a full-service architecture and design firm. From the first brick of construction to the final piece of bespoke furniture, we handle every detail with precision and passion.
            </p>
            <div className="grid grid-cols-2 gap-4 md:gap-8 mb-6 md:mb-10">
              <div className="p-4 md:p-6 bg-gray-50 rounded-2xl border-l-4 border-accent">
                <span className="block text-3xl md:text-4xl font-bold text-primary mb-1">500+</span>
                <span className="text-[8px] md:text-[10px] font-bold uppercase tracking-widest text-gray-400">Projects Completed</span>
              </div>
              <div className="p-4 md:p-6 bg-gray-50 rounded-2xl border-l-4 border-accent">
                <span className="block text-3xl md:text-4xl font-bold text-primary mb-1">10+</span>
                <span className="text-[8px] md:text-[10px] font-bold uppercase tracking-widest text-gray-400">Years Excellence</span>
              </div>
            </div>
          </div>
        </div>
      ...
      <section className="py-12 md:py-24 bg-primary text-white overflow-hidden relative">
        ...
        <div className="container mx-auto px-4 relative z-10">
          <div className="text-center mb-10 md:mb-16">
            ...
            <p className="text-sm md:text-base text-white/60 max-w-2xl mx-auto px-4">We offer a 360-degree approach to architecture and interiors, ensuring quality and luxury at every step.</p>
          </div>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            {services.map((service, index) => (
              <Card key={index} className="glass-card bg-white/10 border-white/20 hover:bg-white hover:text-primary group transition-all duration-500 overflow-hidden flex flex-col px-0 pt-0">
                <div className="relative w-full h-48 md:h-56 overflow-hidden border-b border-white/10 group-hover:border-primary/10">
                   <Image 
                     src={service.image} 
                     alt={service.name} 
                     fill 
                ...
                <CardContent className="p-6 md:p-8 flex-1 flex flex-col">
                  <h3 className="text-xl md:text-2xl font-bold mb-3 md:mb-4 uppercase tracking-tight">{service.name}</h3>
                  <p className="text-white/60 group-hover:text-gray-600 mb-6 md:mb-8 text-sm md:text-base flex-1">{service.desc}</p>
                  <Link href={`/services/${service.slug}`} className="text-accent group-hover:text-primary font-bold flex items-center gap-2 text-[10px] md:text-xs tracking-widest mt-auto">
      ...
      <section className="py-12 md:py-24 bg-gray-50 overflow-hidden relative">
        <div className="container mx-auto px-4 relative z-10">
          <div className="text-center mb-10 md:mb-16">
            ...
            <h2 className="text-3xl md:text-6xl font-bold mb-4 md:mb-6 uppercase tracking-tighter text-primary">Premium Furniture</h2>
            <p className="text-sm md:text-base text-gray-500 max-w-2xl mx-auto px-4 mb-8">
              From ergonomic office setups to luxurious home comfort, we provide bespoke furniture that defines your space.
            </p>
            
            {/* Filter Buttons */}
            <div className="flex justify-center gap-2 mb-10 md:mb-12">
              {['All', 'Home', 'Office'].map((filter) => (
                <button
                  key={filter}
                  onClick={() => setFurnitureFilter(filter)}
                  className={cn(
                    "px-6 md:px-8 py-2 md:py-3 rounded-full text-xs md:text-sm font-bold transition-all border-2",
                    furnitureFilter === filter 
                      ? "bg-primary text-white border-primary shadow-lg" 
                      : "bg-white text-primary border-gray-200 hover:border-primary/30"
                  )}
                >
                  {filter}
                </button>
          ...
          <div className="mt-16 text-center">
            <Link href="/contact">
              <Button size="lg" className="bg-accent hover:bg-accent/90 text-primary font-black px-10 py-7 text-lg rounded-2xl shadow-xl transition-all hover:scale-105 active:scale-95 group">
                GET CUSTOM FURNITURE <ChevronRight className="w-6 h-6 ml-2 group-hover:translate-x-2 transition-transform" />
              </Button>
            </Link>
          </div>
        </div>
