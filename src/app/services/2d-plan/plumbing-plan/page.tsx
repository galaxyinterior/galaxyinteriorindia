...
import { ChevronRight, CheckCircle2, Wrench, LayoutGrid, Check } from "lucide-react";
import Image from "next/image";
import Link from "next/link";

const SPECS = [
  { label: "Piping Material", value: "Class 1 CPVC (Hot) / UPVC (Cold)" },
  { label: "Drainage Slope", value: "1:50 / 1:100 Minimum Gradient" },
  { label: "File Formats", value: "PDF, DWG (AutoCAD), PNG" },
  { label: "Plumbing Code", value: "National Building Code (NBC) Compliant" },
];

const FEATURES = [
  {
    icon: <Wrench className="w-5 h-5" />,
    title: "Hot & Cold Water Routing",
    desc: "Routing twin piping tracks from centralized geysers and solar tanks to individual mixers, faucets, and showers.",
  },
  {
    icon: <LayoutGrid className="w-5 h-5" />,
    title: "Slope & Gravity Mapping",
    desc: "Calculating precise gravity slopes (1% to 2%) for soil and waste pipes to ensure smooth gravity drainage without blocks.",
  },
  {
    icon: <Wrench className="w-5 h-5" />,
    title: "Sanitary Bracket Positioning",
    desc: "Specifying exact installation heights and center-to-center alignments for wall-hung closets, flush tanks, and counter basins.",
  },
  {
    icon: <LayoutGrid className="w-5 h-5" />,
    title: "Trap & Odour Control",
    desc: "Detailing anti-syphon loops, multi-floor ventilation stacks, and bottle traps to prevent toxic sewage gases from entering rooms.",
  },
  {
    icon: <Wrench className="w-5 h-5" />,
    title: "Central Manifold Zones",
    desc: "Designing centralized ring mains and booster pump setups to guarantee constant and balanced water pressure across all faucets.",
  },
  {
    icon: <LayoutGrid className="w-5 h-5" />,
    title: "Vertical Piping Shafts",
    desc: "Designating dedicated civil shafts for vertical downtake pipes, keeping external brickwork clean and leakage-free.",
  },
];

const DELIVERABLES = [
  "Internal Hot & Cold Water Supply Piping Route Map",
  "Toilet & Kitchen Drainage Gravity Slope Blueprint",
  "Sanitaryware Bracket Coordinates & Installation Height Chart",
  "Ventilation Stack & Anti-Syphon Piping Diagram",
  "External Inspection Chamber & Sewerage Chamber Layout",
  "Vertical Civil Plumbing Shaft Section Profile Drawing",
  "Original AutoCAD Source File (.DWG Format)",
  "High-Resolution Printable A3 PDF Blueprints",
];

export default function PlumbingPlanPage() {
  return (
    <div className="bg-[#051124] min-h-screen text-white">
      ...
      <section className="relative pt-28 pb-0 overflow-hidden border-b border-white/10">
        <div className="absolute inset-0 z-0">
          <Image
            src="/generated/2d_plumbing_plan.png"
            alt="2D Plumbing Plan Overview"
            fill
        ...
        <div className="relative z-10 container mx-auto px-4 max-w-5xl py-20">
          ...
          <div className="flex items-center gap-2 text-accent/80 font-bold uppercase tracking-widest text-[10px] mb-6">
            ...
            <ChevronRight className="w-3 h-3" />
            <span className="text-accent">2D Plumbing Plan</span>
          </div>
          ...
          <Badge className="mb-6 rounded-full bg-accent text-primary font-black tracking-[0.20em] px-6 py-2 border-none shadow-md text-[10px] uppercase">
            Supply & Drainage Routes
          </Badge>
          <h1 className="font-display text-5xl md:text-8xl font-black tracking-tight mb-6 uppercase leading-none">
            2D <span className="text-gold italic">Plumbing</span> Plan
          </h1>
          <p className="max-w-2xl text-base md:text-lg text-white/60 font-semibold leading-relaxed mb-10">
            Prevent leakages and ensure optimal water pressure forever. We design structural down-takes, exact drainage slopes, hot/cold supply manifolds, and vent stacks prior to plastering.
          </p>
          <div className="flex flex-wrap gap-4">
            <Button asChild className="rounded-full px-6 md:px-10 h-11 md:h-14 text-xs md:text-sm font-black uppercase tracking-[0.15em] bg-accent hover:bg-accent/90 text-primary shadow-lg shadow-accent/20 hover:scale-105 active:scale-95 transition-all">
              <Link href="/contact?service=2D%20Plumbing%20Plan">Get a Free Quote</Link>
            </Button>
        ...
        <div className="container mx-auto px-4">
          <div className="text-center mb-12">
            <Badge className="mb-4 rounded-full bg-white/5 text-accent font-black tracking-[0.20em] px-5 py-2 border border-accent/20 text-[10px] uppercase">
              Sanitaryware Layouts
            </Badge>
            <h2 className="text-3xl md:text-5xl font-black uppercase tracking-tight text-white">
              Water <span className="text-gold italic">Distribution</span>
            </h2>
          </div>
          ...
          <div className="max-w-4xl mx-auto rounded-[32px] overflow-hidden border border-white/10 bg-[#0a1f3d] p-6 shadow-2xl shadow-black/50">
            <div className="relative aspect-[4/3] w-full bg-white rounded-[20px] overflow-hidden p-4">
              <Image
                src="/generated/2d_plumbing_plan.png"
                alt="Detailed 2D Plumbing Plan Drawing"
                fill
                className="object-contain"
              />
            </div>
            <div className="mt-6 flex flex-wrap gap-3 justify-center">
              <span className="bg-[#1a2d4d]/90 text-[#5b9bd5] text-[10px] font-black uppercase tracking-widest px-3 py-1 rounded-full border border-[#5b9bd5]/30">Plumbing CAD Drawing</span>
              <span className="bg-[#1a2d4d]/90 text-accent text-[10px] font-black uppercase tracking-widest px-3 py-1 rounded-full border border-accent/30">Hot & Cold Loops Mapped</span>
              <span className="bg-[#1a2d4d]/90 text-emerald-400 text-[10px] font-black uppercase tracking-widest px-3 py-1 rounded-full border border-emerald-400/30">Zero post-slab blockages</span>
            </div>
          </div>
        </div>
      </section>
        ...
        <div className="container mx-auto px-4">
          <div className="text-center mb-14">
            ...
            <p className="text-white/50 mt-4 text-sm font-semibold max-w-lg mx-auto">
              Every professional Plumbing Plan drafted by the Galaxy Interior design studio includes these core engineering standards.
            </p>
          </div>
          ...
          <div className="grid md:grid-cols-2 gap-12 items-center">
            <div>
              ...
              <Button asChild className="mt-10 rounded-full px-6 md:px-10 h-11 md:h-14 text-xs md:text-sm font-black uppercase tracking-[0.15em] bg-accent hover:bg-accent/90 text-primary shadow-lg shadow-accent/20 hover:scale-105 active:scale-95 transition-all">
                <Link href="/contact?service=2D%20Plumbing%20Plan">Enquire For Plumbing Plan</Link>
              </Button>
            </div>
            <div className="relative rounded-[32px] overflow-hidden aspect-square border border-white/10">
              <Image
                src="/generated/2d_plumbing_plan.png"
                alt="Plumbing slope routing draft"
                fill
